...
├── temp_split.py            # Utility script for dataset splitting/debugging
├── requirements.txt
│
├── core/                    # Shared Library Code
│   ├── model.py             # Digit CNN classifier
│   ├── dataset.py           # Digit dataset + transforms
│   ├── roi_utils.py         # ROI warp/slicing helpers
│   ├── config.py            # Unified Configuration
│   └── ui/                  # Reusable ROI crop label widgets
│
├── pipelines/               # Automated Processing Scripts
│   ├── train.py             # Model training script
│   ├── inference.py         # Run inference on video
│   ├── evaluate.py          # Validation/test evaluation
│   ├── preparation/         # Extract frames and dataset split scripts
│   └── maintenance/         # Cleanup/stats/heatmap utilities
│
├── workflows/               # GUI Applications
...
│   ├── labels/              # Train/val/test split CSVs (generated by split.py or video_level_split.py)
│   ├── raw_videos/          # Place your MP4s here
│   ├── images/              # Extracted training images (generated by extract.py)
│   └── models/              # Saved checkpoints (generated by training)
│
...
Then choose one split strategy:

```bash
# Row-level stratified split (recommended/default in current workflow)
python pipelines/preparation/split.py

# Video-level split (keeps each video entirely in one split)
python pipelines/preparation/video_level_split.py
```
...
Split script notes:
- `pipelines/preparation/split.py`: row-level stratified splitting across all labelled frames. This is what gets used by the menu.
- `pipelines/preparation/video_level_split.py`: video-level splitting to avoid cross-split leakage from the same source video.
...
The current model is a digit classifier.

1. **ROI warping**: ROI metadata defines a quadrilateral section of the scale display per video segment.
2. **Digit slicing**: Each warped ROI is split using four divider coordinates into five horizontal regions, then the decimal-point region is skipped, leaving four digit crops.
3. **CNN Backbone**: A compact CNN processes each grayscale digit crop with convolution, batch normalization, ReLU, pooling, and adaptive average pooling.
4. **Classifier head**: A two-layer MLP head (with dropout + ReLU) predicts one of 10 digit classes per crop.
...
What is saved to exported video vs preview-only:
- **Saved to exported video**: trim range, crop, warp crop (perspective), brightness, contrast, saturation, and temporal averaging.
- **Preview-only (not currently supported by video exporting)**: CLAHE toggle and gamma slider.
- **Export backend note**: non-warp exports require system `ffmpeg`; warp-enabled exports use the OpenCV rendering path.
