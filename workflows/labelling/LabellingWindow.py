...
class LabellingWindow(QWidget):
    ...
    def __init__(self, video_capture, video_filename: str, start_frame: int, 
                 ...
                 total_videos: int = 1, covered_ranges: list | None = None):
        """
        ...
        Args:
            ...
            start_frame: Valid range start (overall start)
            end_frame: Valid range end (overall end)
            ...
            covered_ranges: List of (start, end) tuples for ROI-covered frame ranges.
                            If provided, only sample from these ranges.
        """
        super().__init__()
        ...
        # Use covered_ranges for sampling if provided
        self.target_frames, self.interval, self.warning = self._calculate_target_frames()
        
        self.frame_data = {}  # {frame_num: weight_string}
        self.labels_entered = 0
        
        self.result_action = VideoCompleteDialog.NEXT_VIDEO # for batch manager
        ...
        self.slider_timer.setInterval(50)
    ...
    def setup_main_ui(self) -> QVBoxLayout:
        ...
        self.video_label.setFocusPolicy(Qt.FocusPolicy.StrongFocus)  # Accept wheel and drag events to allow zooming and panning
        ...
        weight_label = QLabel(f"Weight ({int(self.WEIGHT_MIN)}-{int(self.WEIGHT_MAX)}):")
        self.weight_entry = QLineEdit()
        self.weight_entry.setPlaceholderText(f"Enter weight ({int(self.WEIGHT_MIN)}-{int(self.WEIGHT_MAX)}, up to 3 decimals)")
    ...
    def wheelEvent(self, event):
        """Redirect wheel events to video label if mouse is over it."""
    ...
    def validate_weight(self, value: str) -> tuple[bool, str]:
        ...
        try:
            num = float(value)
            if num < self.WEIGHT_MIN or num > self.WEIGHT_MAX:
                return False, f"Weight must be between {int(self.WEIGHT_MIN)} and {int(self.WEIGHT_MAX)}"
    ...
    def _save_current_weight_if_valid(self, show_warning: bool = False) -> bool:
        """Save current weight entry if valid. returns True if saved"""
    ...
    def update_progress_display(self):
        ...
        self.target_display.setText(f"Target frame: {self.target_frame_index + 1} / {len(self.target_frames)}")
    ...
    def _show_complete_dialog(self):
        ...
        if self.result_action == VideoCompleteDialog.EXIT_BATCH:
            ...
            # stay open so user can review the labels
            pass
        else:  # next
            self.close()
    ...
    def _calculate_target_frames(self) -> tuple[List[int], int, bool]:
        """
        Calculate target frames for labelling.
        
        Returns:
            tuple: (target_frames list, interval m, warning_flag)
        """
        if self.start_frame < 0 or self.end_frame >= self.total_frames:
            raise ValueError("Valid range must be within video bounds")
        if self.start_frame >= self.end_frame:
            raise ValueError("valid_start must be less than valid_end")
        if self.target_labels <= 0:
            raise ValueError("target_labels must be positive")

        if self.covered_ranges:
            valid_frame_count = sum(end - start + 1 for start, end in self.covered_ranges)
        else:
            valid_frame_count = self.end_frame - self.start_frame + 1

        interval_between_labels = math.floor(valid_frame_count / self.target_labels)
        ...
        if self.covered_ranges:
            target_frames = self._sample_from_covered_ranges(interval_between_labels)
        else:
            current_frame = self.start_frame
            while current_frame <= self.end_frame and len(target_frames) < self.target_labels:
    ...
    def _sample_from_covered_ranges(self, interval: int) -> List[int]:
        ...
        for start, end in sorted(self.covered_ranges):
            all_valid_frames.extend(range(start, end + 1))
        ...
        target_frames = []
        idx = 0
        while idx < len(all_valid_frames) and len(target_frames) < self.target_labels:
            target_frames.append(all_valid_frames[idx])
            idx += interval
