...
Uses a greedy video-level assignment to get close to target split ratios while
preserving leakage safety by assigning each whole video to a single split.

Note: this split script is not default because it got ~40% accuracy whereas
split.py (row stratification) got ~40%. Results may differ with different datasets.
"""
import argparse
import re
import sys
